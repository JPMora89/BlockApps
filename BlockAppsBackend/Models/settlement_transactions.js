...
class SettlementTransaction {
  ...
  static async create(subsidiaryId, amount) {
    const queryString = 'INSERT INTO settlement_transactions (subsidiary_id, amount) VALUES ($1, $2) RETURNING *';
  ...
  static async getBySubsidiaryId(subsidiaryId) {
    const queryString = 'SELECT * FROM settlement_transactions WHERE subsidiary_id = $1';
  ...
  static async update(id, amount) {
    const queryString = 'UPDATE settlement_transactions SET amount = $1 WHERE id = $2 RETURNING *';
  ...
  static async delete(id) {
    const queryString = 'DELETE FROM settlement_transactions WHERE id = $1 RETURNING *';
    const values = [id];
    try {
      const { rows } = await db.query(queryString, values);
      return rows[0];
