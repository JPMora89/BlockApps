...
class Subsidiary {
  ...
  static async create(name, budget) {
    const queryString =
      "INSERT INTO subsidiaries (name, budget) VALUES ($1, $2) RETURNING *";
  ...
  static async update(subsidiaryId, name, budget) {
    const queryString =
      "UPDATE subsidiaries SET name = $1, budget = $2 WHERE subsidiary_id = $3 RETURNING *";
  ...
  static async softDelete(subsidiaryId) {
    const queryString =
      "UPDATE subsidiaries SET deleted = TRUE WHERE subsidiary_id = $1 RETURNING *";
    const values = [subsidiaryId];
    try {
      const { rows } = await db.query(queryString, values);
      return rows[0];
  ...
  static async delete(subsidiaryId) {
    const queryString =
      "DELETE FROM subsidiaries WHERE subsidiary_id = $1 RETURNING *";
    const values = [subsidiaryId];
    try {
      const { rows } = await db.query(queryString, values);
      return rows[0];
