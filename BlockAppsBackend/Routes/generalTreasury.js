...
router.get('/getTotalPool', async (req, res) => {
    try {
      // Call the method to retrieve the last exchange rate from the database
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (!record) {
      return res.status(404).json({ error: 'General treasury record not found' });
    }
    res.json(record);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
// Route to retrieve the last exchange rate
